...
class Scan_Page():
    ...
    def build_main_ops(self):
        ...
        p.entries["z_focus"] = page_helpers.createSingleEntry(p.third_row, "Z-focus:", min_val = 0, max_val = 20)

        p.fourth_row = page_helpers.createFrame(p, "third_row", [0.13, 0.34, 0.45, 0.05])
        p.entries["resolution"] = page_helpers.createSingleEntry(p.fourth_row, "Resolution:", min_val = 0.5, max_val = 25)
